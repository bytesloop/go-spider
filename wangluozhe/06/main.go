...
// 执行js文件
func execjs() string {
	bytes, err := ioutil.ReadFile("4.js")
	check(err)
	vm := otto.New()
...
func get_signature(page int) int {
	...
	method := "POST"
	payload := strings.NewReader(`page=` + strconv.Itoa(page) + `&count=10`)
	client := &http.Client{}
	...
	req.Header.Add("Cookie", "session=067ab14c-6782-492d-b896-dcfe84af7d8e.OMiVVCs55dHWTPWE9vCb4AzSVj4; v="+hexin)
	req.Header.Add("hexin-v", hexin)
